...
import { canWriteToSupabase, getSupabase, isSupabaseConfigured, warnNotConfigured, } from "./supabase.js";
...
function toTourRow(tour, sortOrder) {
    return {
        slug: tour.slug,
        name: tour.name,
...
function toVehicleRow(vehicle, sortOrder) {
    return {
        slug: vehicle.slug,
        name: vehicle.name,
...
function readFailed(entity, error) {
    console.error(`[content-store] Read failed for ${entity}, serving the JSON fallback. ` +
        "Has schema.sql been run?", error);
}
...
function requireWritableClient() {
    ...
    if (!supabase || !canWriteToSupabase()) {
        throw new ContentStoreError("Supabase is not configured for writes. Set NEXT_PUBLIC_SUPABASE_URL and " +
...
async function ensureSeeded(supabase) {
    if (seedChecked || !canWriteToSupabase())
        return;
    seedChecked = true;
    try {
        ...
        await supabase
            .from("tours")
            .upsert(seedTours.map((tour, index) => toTourRow(tour, index)), { onConflict: "slug" });
        await supabase
            .from("vehicles")
            .upsert(seedVehicles.map((vehicle, index) => toVehicleRow(vehicle, index)), { onConflict: "slug" });
        await supabase
            .from("app_meta")
...
export async function saveTour(tour, originalSlug) {
...
export async function deleteTourBySlug(slug) {
    if (!isSupabaseConfigured()) {
...
export async function saveVehicle(vehicle, originalSlug) {
    if (!isSupabaseConfigured()) {
    ...
    if (originalSlug) {
        const { error } = await supabase
            ...
            .eq("slug", originalSlug);
        if (error)
            throw writeError("vehicles", error);
        return vehicle;
    }
...
export async function deleteVehicleBySlug(slug) {
    if (!isSupabaseConfigured()) {
...
export async function resetToSeedData() {
    ...
    if (deleteTours.error)
        throw writeError("tours", deleteTours.error);
    const deleteVehicles = await supabase.from("vehicles").delete().neq("slug", "");
    if (deleteVehicles.error)
        throw writeError("vehicles", deleteVehicles.error);
    const insertTours = await supabase
        .from("tours")
        .insert(seedTours.map((tour, index) => toTourRow(tour, index)));
    if (insertTours.error)
        throw writeError("tours", insertTours.error);
    const insertVehicles = await supabase
        .from("vehicles")
        .insert(seedVehicles.map((vehicle, index) => toVehicleRow(vehicle, index)));
    if (insertVehicles.error)
        throw writeError("vehicles", insertVehicles.error);
    await supabase
        .from("app_meta")
        .upsert({ key: "content_seeded", value: "true" }, { onConflict: "key" });
